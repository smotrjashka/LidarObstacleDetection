...
void clusterHelper(int indx, const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& processed,
    ...
    for (int id : nearest) {
        ...
        if (!processed[id])
        {
            ...
            clasterHelper(id, points, claster, processed, tree, distanceTol);
        }
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
    ...
    int sizeReserved = points.size()/2;
        ...
        for (int i = 0; i < ponts.size(); i++)
        {
            ...
            claster.reserve(sizeReserved);

            clasterHelper(i, points, claster, processed, tree, distanceTol);
            clusters.push_back(claster);


        }

        clasters.emplace_back(cluster);
